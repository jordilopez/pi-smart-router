...
describe("resolveRoute: thresholds", () => {
  ...
  it("uses default thresholds when cheapMax is omitted (compat)", () => {
    // No classifier.thresholds at all: cheap <= 0.18, fast <= 0.35,
    ...
    expect(resolveRoute(registry(), cfg, features({ complexityScore: 0.18 }), ctx).route).toBe("cheap-code");
    expect(resolveRoute(registry(), cfg, features({ complexityScore: 0.19 }), ctx).route).toBe("fast");
    expect(resolveRoute(registry(), cfg, features({ complexityScore: 0.35 }), ctx).route).toBe("fast");
    expect(resolveRoute(registry(), cfg, features({ complexityScore: 0.36 }), ctx).route).toBe("balanced");
...
describe("resolveRoute: escalation tier override", () => {
  ...
  it("classifierThresholds + tierForScore match the resolver's tier computation", () => {
    ...
    expect(t).toEqual({ cheapMax: 0.18, simpleMax: 0.35, mediumMax: 0.8 });
    expect(tierForScore(0.1, t)).toBe("cheap");
    expect(tierForScore(0.18, t)).toBe("cheap");
    expect(tierForScore(0.25, t)).toBe("fast");
    expect(tierForScore(0.35, t)).toBe("fast");
    expect(tierForScore(0.36, t)).toBe("balanced");
